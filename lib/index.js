...
class SimplePicker {
  ...
  clearRows() {
    this.$tds.forEach((td) => td.innerHTML = '');
  }
  ...
  handleIconButtonClick(el) {
    ...
    if (selectedDate) {
      const $dateTd = this.findElementWithDate(selectedDate);
      this.selectDate($dateTd);
    }
  }

  initListeners() {
    const { $simplepicker, $timeInput } = this;
    const _this = this;
  ...
  show() {
    this.$simplepicker.classList.add('active');
  }

  // can be called by user or by click the cancel btn.
  hide() {
    this.$simplepicker.classList.remove('active');
  }
